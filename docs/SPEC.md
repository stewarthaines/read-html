...
- Both build targets carry the slot marker byte-for-byte, exactly once, plus the `readhtml-version` meta fed from `package.json` (now a real version, starting 0.1.0).
...
- **Threat model (stated per §3.4)**: consented book scripts run with the app origin's authority — the OPFS/IndexedDB library, localStorage, all of it. This is accepted by design for a trusted-publisher reader on a dedicated origin holding nothing but reading data; the consent gate exists so that authority is only ever granted explicitly, per book, by the reader.
  - **Amendment (post-M7, deployment change): the dedicated-origin premise no longer holds, and this is accepted.** READ.html ships at `readitinabook.com/read/`, sharing an origin with SEED.html — a path prefix is not an origin boundary, so a consented book's scripts run with authority over the **whole `readitinabook.com` origin's** script-writable storage, including SEED.html's, not just reading data. **Owner decision (accepted, 2026-07-17):** run on the shared origin as-is. Rationale: this is a first-party trusted-publisher tool; reader/editor scripts hold nothing sensitive in JS-readable same-origin storage on that origin (no auth tokens or private data a book script could meaningfully read, exfiltrate, or clobber), and catalog trust is not a concern here. The consent gate remains the boundary that matters — book scripts run only on explicit per-book consent (or trusted-catalog auto-consent), never by default. The subdomain option (full origin isolation) was judged over-engineering given nothing sensitive lives on the origin. If that ever changes — SEED.html starts holding auth tokens or sensitive client state on `readitinabook.com`, or trusted catalogs point at untrusted third parties — revisit and move READ.html to `read.readitinabook.com` (isolates origin-scoped storage at zero feature cost).
