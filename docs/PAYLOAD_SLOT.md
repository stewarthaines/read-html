...
- **Browser reach**: a payload copy opened from disk (`file://`) renders in Firefox and Safari but not Chrome/Chromium (the engine's `blob:` resources are blocked in a `file:`-origin page — see `vendor/foliate-js/VENDORED.md`). A payload copy served over http(s) works in every browser. Publishing tools that expect Chrome-from-double-click should host the file rather than ship it for disk opening.
