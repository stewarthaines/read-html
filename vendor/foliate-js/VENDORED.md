...
## Known limitation: single-file `READ.html` from `file://` in Chromium

The single-file build opened directly from disk (`file://`) renders books in Firefox and Safari but **not Chromium/Chrome**: foliate loads each section resource as a `blob:` URL, and Chromium refuses to load a `blob:null/…` resource inside a `file:`-origin iframe (`Not allowed to load local resource`). This is Chromium security policy for opaque `file:` origins, not a foliate defect, and cannot be fixed in vendored code without replacing the engine's `blob:`-based resource loader (data:/srcdoc), which would break its random-access reads and the scripting model. Served over http(s) — the hosted app and any hosted single-file/payload copy — all engines work. Documented in the README and `docs/PAYLOAD_SLOT.md`.
