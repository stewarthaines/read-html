...
5. **`paginator.js`: `expand()` returns early when the view has no document** — the per-view ResizeObserver fires while the iframe is still loading (Firefox, during construction) and from queued callbacks after teardown, and `expand()` destructured `this.document` (null) both times, spraying uncaught TypeErrors on every open/close. Worth reporting upstream.
6. **`paginator.js`, `fixed-layout.js`: sections are delivered via `iframe.srcdoc`, not `iframe.src = blob:`** — Chrome refuses to navigate an iframe to a `blob:null/…` resource under a `file:` (opaque) parent origin, so the single-file build opened from disk showed a blank reader in Chrome/Chromium (`Not allowed to load local resource`). Only top-level frame *navigation* to `blob:null` is blocked; `srcdoc`, and the `blob:` subresources (CSS, images, fonts, the book's own `<script src>`) it references, load fine in every engine on `file://` — verified by a spike covering rendering, cross-section/TOC navigation, and scripted playback. The patch fetches the section blob back to a string (the exact bytes it holds) and sets `srcdoc`; it falls back to `src` for any non-`blob:` URL. Applies to all origins (not just `file:`), so the one code path is exercised everywhere. Worth reporting upstream.
